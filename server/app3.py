...
def get_task_result():
   ...
   # Poll for task status until it's no longer 'PENDING'
    ...
    # Task is no longer pending, return the result or error
...
def generate_dpia(self, data):
    ...
    with app.app_context():
        ...
        retriever = partition_process(UP_DIR_R, data.get('user_id', ''), str(project_id), file_names, IMG_DIR_R, IMG_DIR_RD, partition_model, embeddings, 'n/a', id_key, file_name, embed_type, usage, f"project_{project_id}")
